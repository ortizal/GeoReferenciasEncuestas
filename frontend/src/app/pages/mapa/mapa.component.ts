...
export class MapaComponent implements AfterViewInit, OnDestroy {
  ...
  loadManzanas() {
    this.manzanaService.listarConPoligono().subscribe({ next: (r) => { if (r.exitoso) { this.manzanas.set(r.datos); this.renderManzanas(); } } });
  }

  loadPredios() {
    this.predioService.listarConGeoreferencia().subscribe({ next: (r) => { if (r.exitoso) { this.predios.set(r.datos); this.renderPredios(); } } });
  }
